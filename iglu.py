...
# OBJECT CASTING AND DEFINITION OF OPERATIONS
# Define class of object exam
class exam:
    data = None
...
def print_table(table):
    ...
    for row in table:
        if row[0] in years_to_int.keys():
            ...
            print("{0:>55}{1:^7}{2:^12}{3:^12}".format(row[1], row[2], row[3], row[4]))
...
# On \110 base
try:
    graduation = round(waverage(record_avg)*11/3)
...
def mode(record_avg):
    ...
    return f'{str(mode)}: ne ha presi ben {str(mfreq)}, il {str(perc)}% sul totale degli esami.'
...
def best_year(record):
    ...
    cfu_list = [(obj.date,obj.cfu,obj.year) for obj in record if obj.name != 'Prova finale']
...
while True: 
    ...
    if user_input == '0':
        ...
        print(f'\nSiamo ansiosetti, eh? Da questa media dipenderà il Suo voto di Laurea.\nVediamo, {waverage(record_avg)} su 30. {judging(record_avg)}, si presenta alla discussione della Tesi con un punteggio di {graduation} su 110 {cum_laude(record)}.\nAncora!',end='\n\n')
        for entry in options:
            if entry != user_input:
                print(entry, menu[entry])
    elif user_input == '3':
        print(f'\nIl Suo voto mediano è {median(record_avg)}, ma non si monti la testa: la Sua vita in Università sarà un rincorrere asintoticamente la perfezione.\nFioccano i {mode(record_avg)}',end='\n\n')
        for entry in options:
            if entry != user_input:
                print(entry, menu[entry])
    elif user_input == '4':
